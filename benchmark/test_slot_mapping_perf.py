...
class SlotMappingBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype) -> Generator:
        for nreq, maxtok, bs in self.shapes:
            tlen = 200
            qsl = torch.tensor([0] + [tlen * (i + 1) for i in range(nreq)], dtype=torch.int32,
                               device=self.device)
            pos = torch.cat([torch.arange(tlen, dtype=torch.int32) for _ in range(nreq)]).to(self.device)
            maxb = (16384 + bs - 1) // bs + 1
            btab = (torch.arange(nreq * maxb, dtype=torch.int32).reshape(nreq, maxb) % 900).to(self.device)
...
def _gems(num, maxtok, qsl, nreq, pos, btab, bs):
    ...
    flaggems_vllm.compute_slot_mapping_parallel(num, maxtok, qsl, nreq, pos, btab, out, block_size=bs)
    return out

def _torch_ref(num, maxtok, qsl, nreq, pos, btab, bs):
    out = torch.empty((maxtok,), dtype=torch.int64, device=qsl.device)
    rows = torch.repeat_interleave(torch.arange(nreq, device=qsl.device), qsl[1:] - qsl[:-1])
...
def _upstream(num, maxtok, qsl, nreq, pos, btab, bs):
    ...
    _upstream_slot_kernel[(nreq + 1,)](
        num, maxtok, qsl, pos, btab, btab.stride(0), bs, out,
        BLOCK_SIZE=1024, TOTAL_CP_WORLD_SIZE=1, TOTAL_CP_RANK=0,
        CP_KV_CACHE_INTERLEAVE_SIZE=1, PAD_ID=-1,
    )
    return out
...
def test_slot_mapping_perf():
    ...
    SlotMappingBenchmark(op_name=op_name, torch_op=torch_op, gems_op=_gems,
                   dtypes=[torch.int32]).run()
