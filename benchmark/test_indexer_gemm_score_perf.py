...
def _matvec(qbar, cache, cum_q, seq_lens, bt, positions, token_start=0):
    ...
    num_chunks = (max_pool + _mod.TRITON_POOL_CHUNK_SIZE - 1) // _mod.TRITON_POOL_CHUNK_SIZE
    _mod._glm5_next_lightning_indexer_score_kernel[(num_tokens, num_chunks)](
        ...
        _mod._next_power_of_2(max(1, num_reqs)), 128, 4,
        _mod.TRITON_POOL_CHUNK_SIZE, _mod.TRITON_POOL_SUBTILE_SIZE,
    )
    return out
...
def _torch_reference(qbar, cache, cum_q, seq_lens, bt, positions):
    ...
    max_pool = int(seq_lens.max().item())
    ends = cum_q
    req = (torch.arange(num_tokens, device=qbar.device)[:, None] >= ends[None, :]).sum(1)
