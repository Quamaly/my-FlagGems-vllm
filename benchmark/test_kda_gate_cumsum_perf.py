...
def _chunk_indices_host(cu_host, chunk):
    ...
    return torch.tensor(idx, dtype=torch.int64) if idx else torch.zeros((0, 2), dtype=torch.int64)
...
class KdaGateCumsumBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype) -> Generator:
        for i, lens in enumerate(self.shapes):
            ...
            g = (torch.rand(1, T, H, D, dtype=torch.float32, device=self.device) * 6 - 3).to(dtype)
            A_log = torch.rand(H, dtype=torch.float32, device=self.device) - 2.0
            dt_bias = torch.rand(H * D, dtype=torch.float32, device=self.device) * 0.4 - 0.2
