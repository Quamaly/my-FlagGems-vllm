...
class PagedScatterBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype) -> Generator:
        for i, (shape, n, bs) in enumerate(self.shapes):
            ...
            total = shape[0] * bs
            slots = torch.randperm(total, device="cpu")[: min(n, total)].to(torch.int64).to(self.device)
            if slots.numel() < n:
...
def test_paged_scatter_perf():
    PagedScatterBenchmark(op_name="paged_scatter", torch_op=_torch_scatter, gems_op=_gems_scatter,
                   dtypes=[torch.bfloat16]).run()
