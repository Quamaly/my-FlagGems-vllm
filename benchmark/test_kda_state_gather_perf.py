...
pytestmark = pytest.mark.skipif(
    not _NPU_AVAILABLE, reason="kda_state_gather requires an Ascend NPU"
)
...
class KdaStateGatherBenchmark(base.Benchmark):
    ...
    def get_input_iter(self, dtype) -> Generator:
        for i, (S, N, H, V, K) in enumerate(self.shapes):
            ...
            has = (torch.rand(N) > 0.3).to(torch.int32).to(self.device)
            yield cache, idx, has


def _torch_gather(cache, idx, has):
    st = cache[idx]
    return (
        torch.where((has > 0).view(-1, 1, 1, 1), st, torch.zeros_like(st))
        .transpose(-1, -2)
        .contiguous()
    )
...
def test_kda_state_gather_perf():
    KdaStateGatherBenchmark(
        op_name="kda_state_gather",
        torch_op=_torch_gather,
        gems_op=flaggems_vllm.gather_kda_state,
