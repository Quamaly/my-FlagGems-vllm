...
@triton.jit(do_not_specialize=["num_rows", "num_slots", "inner_numel", "stride_block", "stride_off", "value_stride"])
    ...
    for i in tl.static_range(ROWS):
        ...
        if n < num_rows:
            ...
            if (slot >= 0) & (slot < num_slots):
                ...
                v = tl.load(values_ptr + n.to(tl.int64) * value_stride + o, mask=m_col, other=0)
                tl.store(
                    cache_ptr + row.to(tl.int64) * stride_block + off.to(tl.int64) * stride_off + o,
                    v,
                    mask=m_col,
                )
...
def paged_scatter_triton(
    ...
    _paged_scatter_kernel[(triton.cdiv(n, rows_per_prog), triton.cdiv(inner_numel, block_inner))](
        cache,
        slots,
        values,
        n,
