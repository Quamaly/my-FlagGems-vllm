...
The per-token body is unchanged; each program now handles
``T_PER_PROG`` tokens through a static_range loop and the request-bucketize
loads are hoisted out of the loop.  (A row-wise window rewrite and wider
[rows, inner] tiles were benchmarked and rejected — the paged window gather
keeps this kernel gather-bound on the vector cores.)
"""
...
def _glm5_next_kpool_state_compress_kernel(
    ...
    T_PER_PROG: tl.constexpr,
):
    pid = tl.program_id(0)
    dim_offsets = tl.program_id(1) * BLOCK_D + tl.arange(0, BLOCK_D)
    dim_mask = dim_offsets < HEAD_DIM

    # Request bucketize inputs are loop-invariant: requests are packed
    # contiguously in the batch, so every token in this program's slice
    # compares against the same query_ends vector.
    ...
    for i in tl.static_range(T_PER_PROG):
        token_idx = pid * T_PER_PROG + i
        if token_idx < num_tokens:
            req_id = tl.sum(tl.where(token_idx >= query_ends, 1, 0))
            # Full ACL graphs keep padded rows beyond the last request; keep
            # their pointer arithmetic in bounds even though their stores are
            # masked.
            ...
            # In-window rows live in this launch's k/gate_score inputs; the
            # matching input row is the batch row of the token at that position.
...
def glm5_next_kpool_state_compress_and_write_cache_triton(
    ...
    # T=8 benchmarked best among {1,4,8} (~3-5% over one-token-per-program);
    # larger tiles (16/32) blow the unrolled kernel past AIV limits (vector
    # core exception) and are NOT safe.
    t_per_prog = int(os.environ.get("A2_T_PER_PROG", "8"))
    _glm5_next_kpool_state_compress_kernel[
        (triton.cdiv(num_tokens, t_per_prog), triton.cdiv(head_dim, block_d))
    ](
        state_cache,
        indexer_cache,
        k,
