...
def _glm5_indexer_gemm_score_kernel(
    ...
    page_offsets = pool_offsets % pool_block_size
    physical_blocks = tl.load(
        indexer_block_table_ptr + req * block_table_stride_req + logical_pages * block_table_stride_page,
        mask=in_range,
        other=0,
    ).to(tl.int64)
    physical_blocks = tl.minimum(tl.maximum(physical_blocks, 0), num_cache_blocks - 1)
    ...
    b_tile = tl.load(indexer_cache_ptr + k_addrs, mask=in_range[:, None], other=0.0).to(tl.float32)  # [BN, D]
    ...
    for m0 in tl.range(lo, hi, BM):
        ...
        pos = tl.load(positions_ptr + token_start + offs_m, mask=m_m, other=0).to(tl.int32)
...
def indexer_gemm_score(
    ...
    if num_tokens >= _GEMM_SCORE_MIN_ROWS:
        ...
        _glm5_indexer_gemm_score_kernel[grid](
            qbar, cache, cum_query_lens, seq_lens, block_table, positions, out,
            token_start, max_pool, num_reqs, cache.shape[0], num_tokens,
            cache.stride(0), cache.stride(1), cache.stride(3),
            block_table.stride(0), block_table.stride(1), cache.shape[1],
            req_pow2, head_dim, index_kpool,
            _GEMM_SCORE_BN, _GEMM_SCORE_BM,
        ...
        _glm5_next_lightning_indexer_score_kernel[(num_tokens, num_chunks)](
            qbar, cache, cum_query_lens, seq_lens, block_table, positions, out,
            token_start, max_pool, num_reqs, cache.shape[0],
            cache.stride(0), cache.stride(1), cache.stride(3),
            block_table.stride(0), block_table.stride(1), cache.shape[1],
            req_pow2, head_dim, index_kpool,
            TRITON_POOL_CHUNK_SIZE, TRITON_POOL_SUBTILE_SIZE,
        )
    return out
