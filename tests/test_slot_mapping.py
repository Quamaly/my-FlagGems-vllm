...
def _ref_slot_mapping(num_tokens, qsl_host, positions, block_table, block_size, pad_id=-1):
...
def test_slot_mapping_accuracy(nreq, tlen, maxtok, bs):
    ...
    pos = torch.cat([torch.arange(tlen, dtype=torch.int32) for _ in range(nreq)]).to("npu")
    ...
    btab = (torch.arange(nreq * maxb, dtype=torch.int32).reshape(nreq, maxb) % 900).to("npu")
    expected = _ref_slot_mapping(sum(lens), qsl_host, pos.cpu(), btab.cpu(), bs).to("npu")
...
def test_slot_mapping_matches_upstream_kernel(nreq, tlen, maxtok, bs):
    ...
    _upstream_slot_kernel[(nreq + 1,)](
        sum(lens), maxtok, qsl, pos, btab, btab.stride(0), bs, old,
