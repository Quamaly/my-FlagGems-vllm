...
def build_inputs(tokens_per_req, n_req, max_pool, pos_base=0, seed=0):
    ...
    cum_q = torch.arange(
        tokens_per_req, (n_req + 1) * tokens_per_req, tokens_per_req,
...
def _matvec_reference(qbar, cache, cum_q, seq_lens, bt, positions, kpool=4):
    ...
    out = torch.full((num_tokens, max_pool), float("-inf"), dtype=torch.float32, device=qbar.device)
    ...
    m._glm5_next_lightning_indexer_score_kernel[(num_tokens, num_chunks)](
        qbar, cache, cum_q, seq_lens, bt, positions, out,
        0, max_pool, num_reqs, cache.shape[0],
        cache.stride(0), cache.stride(1), cache.stride(3),
        bt.stride(0), bt.stride(1), cache.shape[1],
        m._next_power_of_2(max(1, num_reqs)), head_dim, kpool,
        m.TRITON_POOL_CHUNK_SIZE, m.TRITON_POOL_SUBTILE_SIZE,
    )
    return out
...
def _assert_single(tokens_per_req, n_req, max_pool, pos_base):
    ...
    actual = flaggems_vllm.indexer_gemm_score(qbar, cache, cum_q, seq_lens, bt, positions)
    ...
    if m.any():
        ...
        if topk > 0:
            ...
            assert int(flips.max()) <= 1, f"topk set mismatch: min inter {int(inter.min())}"
...
def test_indexer_gemm_score_accuracy(tokens_per_req, n_req, max_pool, pos_base):
    ...
    r = subprocess.run(
        [sys.executable, "-c", code, str(tokens_per_req), str(n_req), str(max_pool), str(pos_base)],
        capture_output=True, text=True, timeout=600, env=env, cwd=repo_root,
    )
    assert r.returncode == 0, f"case ({tokens_per_req},{n_req},{max_pool},{pos_base}) failed:\n{r.stderr[-800:]}"
