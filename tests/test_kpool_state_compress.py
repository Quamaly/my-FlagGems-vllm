...
def _reference(state_cache, indexer_cache, k, gate, ape, positions, cum_q, seq_lens,
               state_slots, state_bt, indexer_slots, kpool):
    ...
    req = torch.bucketize(t_idx, cum_q.to(torch.int64), right=True).clamp_max(cum_q.shape[0] - 1)
    ...
    phys = state_bt[req[:, None], page].long().clamp_min(0).clamp_max(state_cache.shape[0] - 1)
...
def _make_case(lens, kpool=4, state_bs=4, indexer_bs=16, hist_prefill=0, pad_slots=False, seed=0):
    torch.manual_seed(seed)
    num_tokens = sum(lens)
    state_cache = torch.zeros(64, state_bs, 2 * HEAD_DIM, dtype=torch.float32, device=DEVICE)
    indexer_cache = torch.zeros(64, indexer_bs, 1, HEAD_DIM, dtype=torch.bfloat16, device=DEVICE)
    ...
    positions = torch.cat([torch.arange(l, device=DEVICE) + hist_prefill for l in lens]).long()
    cum_q = torch.tensor(list(torch.tensor(lens).cumsum(0)), dtype=torch.int32, device=DEVICE)
    seq_lens = torch.tensor([hist_prefill + l for l in lens], dtype=torch.int32, device=DEVICE)
    ...
    state_slots = torch.arange(num_tokens, dtype=torch.int64, device=DEVICE) + hist_prefill + 7
    indexer_slots = torch.arange(num_tokens, dtype=torch.int64, device=DEVICE) + 3
    if pad_slots:
        state_slots[0] = -1
        indexer_slots[1] = -1
    state_bt = torch.arange(64, dtype=torch.int32, device=DEVICE).unsqueeze(0).repeat(len(lens), 1)
    for p in range(hist_prefill):
        state_cache[p // state_bs, p % state_bs] = torch.randn(2 * HEAD_DIM, device=DEVICE)
    return (state_cache, indexer_cache, k, gate, ape, positions, cum_q, seq_lens,
            state_slots, state_bt, indexer_slots, kpool)
...
def test_kpool_state_compress_accuracy(lens, kpool, hist, pad):
    ...
    flaggems_vllm.kpool_state_compress(new_sc, new_ic, k, g, ape, pos, cq, sl, ss, bt, islt, kp)
